...
{
	return m_offset = min((unsigned long)max((unsigned long)m_offset + count, (unsigned long)0), (unsigned long)size() - 1);
}
...
{
	return m_offset = min(max((unsigned long)m_offset + count, (unsigned long)0), (unsigned long)size() - 1);
}

size_t SubFile::read(void*buffer, size_t size)
{
	m_file->seek(m_start + m_offset);
	size = min((unsigned long)size, m_size - m_offset);
...
SubFile::SubFile(IFile*file, const std::string&subfilename, unsigned long start, unsigned long size)
	: IFile(file->name() + L"|" + QString::fromStdString(subfilename).toStdWString())
{
	m_file = file;
	m_start = start;
	m_size = size;
