...
//
	...
	template<> struct TemplateReader<float>   { static float   read(ChunkReader&reader) {
													return reader.readFloat();
												}
	};
	template<> struct TemplateReader<Color>   { static Color   read(ChunkReader&reader) {
													return reader.readColorRGBA();
												}
	};
	template<> struct TemplateReader<Vector3> { static Vector3 read(ChunkReader&reader) {
													return reader.readVector3();
												}
	};
	template<> struct TemplateReader<Vector4> { static Vector4 read(ChunkReader&reader) {
