...
//		bone.invAbsTransform = bone.absTransform.inverse();
	...
	void Model::ReadSubMesh(ChunkReader&reader, SubMesh&mesh, size_t number)
	{
		...
		if (type == 0x10007)
		{
			entryPoint.vertexIn = reader.fileTell();
			reader.read(mesh.vertices, mesh.vertices.size() * sizeof(MASTER_VERTEX));
			entryPoint.vertexOut = reader.fileTell();
		}
		else
		{
		#pragma pack(1)
		...
		entryPoint.indicesIn = reader.fileTell();
		reader.read(mesh.indices, mesh.indices.size() * sizeof(uint16_t));
		entryPoint.indicesOut = reader.fileTell();
		// Read skin mapping
		type = reader.next();
		mesh.nSkinBones = 0;
		if (type == 0x10006)
	...
	Model::Mesh*Model::ReadMesh(ChunkReader&reader)
	{
		...
		for (size_t i = 0; i < mesh->subMeshes.size(); i++)
		{
			...
			ReadSubMesh(reader, mesh->subMeshes[i], i);
	...
	Model::Model(ptr<IFile> file): m_numSubMeshes(0)
	{
		...
		try
		{
			...
			while (type == 0x400 || type == 0x1300)
			{
				if (type == 0x400)
				{
					m_meshesEntryPoints << reader.fileTell();
